 ...
 * https://modernizr.com/download/?-flexbox-flexboxlegacy-flexboxtweener-flexwrap-objectfit-printshiv-setclasses !*/ !(function (
    e,
    t,
    n
) {
    ...
    function i(e) {
        ...
        Modernizr._config.enableClasses &&
            ((t += " " + n + e.join(" " + n)),
            C ? (b.className.baseVal = t) : (b.className = t));
    }
    function a(e) {
        return e
    ...
    function h(e, t, o, i) {
        ...
        for (
            ...
            (c = !0), (j.modElem = l(g.shift())), (j.style = j.modElem.style);
            ...
            function a(e, n, r) {
                ...
                return (
                    (o = r.cache[e]
                        ...
                        : r.createElem(e)),
                    !o.canHaveChildren || x.test(e) || o.tagUrn
                        ? o
...
jQuery(document).ready(function ($) {
    ...
    $(".no-flexbox.no-flexboxlegacy.no-flexboxtweener .Row").wrap(
        '<div class="RowCn"></div>'
    );
    !(function (t) {
        t.fn.percircle = function (a) {
            ...
            return this.each(function () {
                var r = t(this);
                r.hasClass("percircle") || r.addClass("percircle"),
                    "undefined" != typeof r.attr("data-animate") &&
                        (a.animate = "true" == r.attr("data-animate")),
                    a.animate && r.addClass("animate");
    ...
    SearchableOptionList.prototype = {
                ...
                onScroll: function () {
                    var selectionContainerYPos =
                        ...
                        selectionContainerHeight =
                            this.$selectionContainer.outerHeight(false),
                        selectionContainerBottom =
                            selectionContainerYPos + selectionContainerHeight,
                        displayContainerAboveInput =
                            this.config.displayContainerAboveInput ||
            ...
            if (!window[this.WINDOW_EVENTS_KEY]) {
                $(document).click(function (event) {
                    var $clickedElement = $(event.target),
                        $closestSelectionContainer = $clickedElement.closest(
                            ".sol-selection-container"
                        ),
                        $closestInnerContainer = $clickedElement.closest(
                            ".sol-inner-container"
                        ),
                        $clickedWithinThisSolContainer;
                    if ($closestInnerContainer.length) {
                        $clickedWithinThisSolContainer = $closestInnerContainer
                            .first()
        ...
        _initializeUiElements: function () {
            ...
            this.$noResultsItem = $('<div class="sol-no-results"/>')
                ...
                .hide();
            this.$loadingData = $('<div class="sol-loading-data"/>').html(
                this.config.texts.loadingData
            );
            this.$xItemsSelected = $('<div class="sol-results-count"/>');
            this.$caret = $(
                '<div class="sol-caret-container"><b class="sol-caret"/></div>'
            ).click(function (e) {
                self.toggle();
                e.preventDefault();
                return false;
            ...
            this.$selectionContainer = $(
                '<div class="sol-selection-container"/>'
            )
            ...
            if (this.$originalElement.css("display") !== "block") {
                this.$container.css(
                    "width",
                    this._getActualCssPropertyValue(
        ...
        _initializeInputEvents: function () {
            ...
            if (
                $form &&
                $form.length === 1 &&
                !$form.data(this.WINDOW_EVENTS_KEY)
            ) {
                var resetFunction = function () {
                    var $changedItems = [];
                    $form
                        .find(".sol-option input")
                        .each(function (index, item) {
                            var $item = $(item),
                    ...
                    if (
                        ...
                        self.config.events.onChange.call(
                            self,
                            self,
                            $changedItems
                        );
                ...
                .on("keydown", function (e) {
                    ...
                    if (!self.$noResultsItem.is(":visible")) {
                        var $currentHighlightedOption,
                            ...
                            $allVisibleOptions = self.$selection.find(
                                ".sol-option:visible"
                            );
                        if (keyCode === 40 || keyCode === 38) {
                            ...
                            $currentHighlightedOption.removeClass(
                                "keyboard-selection"
                            );
                            $nextHighlightedOption = $(
                            ...
                            $currentHighlightedOption
                                .prop(
                                    "checked",
                                    !$currentHighlightedOption.prop("checked")
                                )
                                .trigger("change");
                            preventDefault = true;
                        }
        ...
        _detectDataFromOriginalElement: function () {
            if (
                ...
                var solDataAttributeValue =
                    this.$originalElement.data("sol-data");
            ...
            var self = this,
                ...
                dataProcessedFunction = function () {
                    ...
                    if ($.isFunction(this.config.events.onInitialized)) {
                        this.config.events.onInitialized.call(
                            this,
                            this,
                            solItems
                        );
                ...
                loopFunction = function () {
                    var currentBatch = 0,
                        item;
                    while (
                        currentBatch++ < self.config.asyncBatchSize &&
                        nextIndex < solItems.length
                    ) {
                        item = solItems[nextIndex++];
                        if (item.type === self.SOL_OPTION_FORMAT.type) {
                            self._renderOption(item);
                        } else if (
                            item.type === self.SOL_OPTIONGROUP_FORMAT.type
                        ) {
                            self._renderOptiongroup(item);
                        } else {
                            self._showErrorLabel(
                                "Invalid item type found " + item.type
                            );
                            return;
                        }
                    }
        ...
        _renderOption: function (solOption, $optionalTargetContainer) {
            var self = this,
                $actualTargetContainer =
                    $optionalTargetContainer || this.$selection,
                $inputElement,
                $labelText = $('<div class="sol-label-text"/>')
                    .html(
                        solOption.label.trim().length === 0
                            ? "&nbsp;"
                            : solOption.label
                    )
            ...
            if (this.config.multiple) {
                $inputElement = $(
                    '<input type="checkbox" class="sol-checkbox"/>'
                );
        ...
        _renderOptiongroup: function (solOptiongroup) {
            var self = this,
                ...
                $groupItem = $('<div class="sol-optiongroup"/>').append(
                    $groupCaption
                );
            if (solOptiongroup.disabled) {
        ...
        _initializeSelectAll: function () {
            if (
                this.config.showSelectAll === true ||
                ($.isFunction(this.config.showSelectAll) &&
                    this.config.showSelectAll.call(this))
            ) {
                var self = this,
                    $deselectAllButton = $(
                        '<a href="#" class="sol-deselect-all"/>'
                    )
        ...
        _selectionChange: function ($changeItem, skipCallback) {
            if (
                ...
                this.$originalElement
                    ...
                    .each(function (index, item) {
                        ...
                        if (
                            $currentOriginalOption.val() === $changeItem.val()
                        ) {
            ...
            var selected = this.$showSelectionContainer.children(
                ".sol-selected-display-item"
            );
            if (
                this.config.maxShow != 0 &&
                selected.length > this.config.maxShow
            ) {
                selected.hide();
                var xitemstext = this.config.texts.itemsSelected.replace(
                    "{$a}",
                    selected.length
                );
                this.$xItemsSelected.html(
                    '<div class="sol-selected-display-item-text">' +
                        xitemstext +
                        "<div>"
                );
        ...
        _addSelectionDisplayItem: function ($changedItem) {
            ...
            if (!$existingDisplayItem) {
                ...
                this.config.scrollTarget.unbind(
                    "scroll",
                    this.internalScrollWrapper
                );
                $(window).off("resize");
                this.$input.val("");
        ...
        selectAll: function () {
            if (this.config.multiple) {
                ...
                if ($.isFunction(this.config.events.onChange)) {
                    this.config.events.onChange.call(
                        this,
                        this,
                        $changedInputs
                    );
        ...
        $("body").delegate("." + strclass, "keyup", function (event) {
            ...
            if (value.length >= 3) {
                ...
                for (var i = 0; i < items.length; i++) {
                    if (
                        items[i].split("|")[0].toLowerCase().indexOf(value) > -1
                    )
                        rm +=
                            '<label><button data-name="' +
                            nameinput +
                            '" data-id="' +
                            items[i].split("|")[1] +
                            '" type="button">' +
