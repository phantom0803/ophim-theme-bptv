    ...
    public function getMovieOfType(Request $request, $slug)
    {
        switch ($slug) {
            case 'phim-de-cu':
                ...
                $movies = Movie::where('is_recommended', 1)->orderBy('created_at', 'desc')->paginate(25);
                break;
            case 'phim-chieu-rap':
                $section_name = 'Phim Chiếu Rạp';
                $movies = Movie::where('is_shown_in_theater', 1)->orderBy('created_at', 'desc')->paginate(25);
                break;
            case 'phim-sap-chieu':
                $section_name = 'Phim Sắp Chiếu';
                $movies = Movie::where('status', 'trailer')->orderBy('created_at', 'desc')->paginate(25);
                break;
            default:
                $type = $slug == 'phim-le' ? 'single' : 'series';
                $section_name = $slug == 'phim-le' ? 'Phim Lẻ' : 'Phim Bộ';
                $movies = Movie::where('type', $type)->orderBy('created_at', 'desc')->paginate(25);
                break;
        }
